...
// Novos objetos inseridos para o trabalho
#define COW           3
#define CAT           4
...
uniform vec4 boundingBox_min;
uniform vec4 boundingBox_max;
...
void main()
{
    ...
    if ( object_id == SPHERE )
    {
        ...
        // "boundingBox_center" definida abaixo.
        ...
        vec4 boundingBox_center = (boundingBox_min + boundingBox_max) / 2.0;
        vec4 max_vec = boundingBox_max;

        max_vec.w = 0.0;

        float rho = length(max_vec);

        vec4 p_vec = normalize(position_model - boundingBox_center) * rho;
        vec4 p_linha = boundingBox_center + p_vec;
    ...
    else if ( (object_id == BUNNY) || (object_id == COW) || (object_id == HOUSE) )
    {
        // PREENCHA AQUI as coordenadas de textura do coelho, computadas com
        // projeção planar XY em COORDENADAS DO MODELO. Utilize como referência
        // o slide 106 do documento "Aula_20_e_21_Mapeamento_de_Texturas.pdf".
        float minx = boundingBox_min.x;
        float maxx = boundingBox_max.x;

        float miny = boundingBox_min.y;
        float maxy = boundingBox_max.y;

        float minz = boundingBox_min.z;
        float maxz = boundingBox_max.z;
    ...
    {
        lambert = 0;  // Caso o ponto não esteja iluminado, usaremos apenas a luz ambiente
    }
